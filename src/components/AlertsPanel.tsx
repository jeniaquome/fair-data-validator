...
export default function AlertsPanel({ isOpen, onClose, onViewReport }: Props) {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div className="fixed sm:absolute inset-x-2 sm:inset-x-auto sm:right-0 top-16 sm:top-full sm:mt-2 w-auto sm:w-96 bg-white rounded-2xl shadow-xl border border-slate-200/60 z-50 overflow-hidden max-h-[80vh] sm:max-h-none flex flex-col backdrop-blur-md">
        ...
        <div className="max-h-96 overflow-y-auto">
          {alerts.map((alert) => (
            <div
              key={alert.id}
              className={`px-5 py-4 border-b border-slate-100 hover:bg-slate-50/50 cursor-pointer transition-all ${
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <p className={`text-sm font-semibold ${!alert.read ? 'text-slate-900' : 'text-slate-700'}`}>
                      {alert.title}
                    </p>
                    {!alert.read && (
                      <span className="w-2 h-2 bg-indigo-600 rounded-full flex-shrink-0 shadow-lg shadow-indigo-600/50" />
                    )}
                  </div>
                  <p className="text-xs text-slate-600 line-clamp-2 mb-2">{alert.message}</p>
                  <p className="text-xs text-slate-400 font-medium">{formatTime(alert.timestamp)}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="px-5 py-3 border-t border-slate-200 bg-slate-50/80">
          <button className="w-full text-center text-sm text-indigo-600 hover:text-indigo-800 font-semibold transition-colors">
...
export function AlertsBadge({ count }: { count: number }) {
  ...
  return (
    <span className="absolute -top-1 -right-1 w-5 h-5 bg-rose-500 text-white text-xs font-bold rounded-full flex items-center justify-center shadow-lg shadow-rose-500/30">
      {count > 9 ? '9+' : count}
    </span>
  )
}
